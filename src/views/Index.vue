                            ...
                            <div class="sidebar">
                                <el-menu
                                    ...
                                    <template v-for="(item,index) in routes">
                                        <el-menu-item v-if="item.children.length <= 1" :index="item.path + `${item.path.length>1? '/':''}` + item.children[0].path" route style="margin-left: 5px">{{item.meta.title}}</el-menu-item>
                                        <el-submenu v-if="item.children.length > 1 && !item.hidden" :index="index + ''"  :key="index">
                                            <template slot="title">
                                              <i style="color: #E4E7ED;margin-right: 5px" :class="item.iconCls"></i>
                                              <span v-text="item.meta.title"></span>
                                            </template>
                                            <el-menu-item :index="item.path + '/' + child.path" v-for="(child,index1) in item.children" :key="index1" route>
                                              <!--<el-menu-item :index="child.path" v-for="(child,index1) in item.children" :key="index1" route>-->
                                              {{child.meta.title}}
                                            </el-menu-item>
                                        </el-submenu>
                                    </template>
...
<script>
    export default {
        ...
        computed: {
            routes() {
                console.warn("this.$store.state.routes;===",this.$store.state.routes)
